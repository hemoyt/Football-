...
function initVideoSection() {
  ...
  addBtn.addEventListener('click', () => {
    const url = prompt('الصق رابط يوتيوب أو فيميو (أو اتركه فارغاً لرفع ملف):');
    if (url === null) return;

    if (url.trim()) {
      addVideoCard({ video_type: detectVideoType(url), video_url: url.trim(), title: 'هايلايت' });
    } else {
  ...
  if (fileInput) {
    fileInput.addEventListener('change', async () => {
      ...
      if (file.size > 500 * 1024 * 1024) {
        renderToast('حجم الفيديو يجب أن يكون أقل من 500 ميغابايت.', 'error');
        return;
      }
      renderToast('جارٍ رفع الفيديو…', 'info');
